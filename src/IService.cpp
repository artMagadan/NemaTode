...
IService::IService(const std::string &prefix):m_prefix(prefix){
}


std::string IService::getTag(const std::string &tag) const {
    return m_prefix + tag;
}


void IService::attachToParser(NMEAParser& _parser){
    ...
    */
    ...
    _parser.setSentenceHandler(getTag("GGA"), [this](const NMEASentence& nmea){
        this->read_XXGGA(nmea);
    });
    _parser.setSentenceHandler(getTag("GSA"), [this](const NMEASentence& nmea){
        this->read_XXGSA(nmea);
    });
    _parser.setSentenceHandler(getTag("GSV"), [this](const NMEASentence& nmea){
        this->read_XXGSV(nmea);
    });
    _parser.setSentenceHandler(getTag("RMC"), [this](const NMEASentence& nmea){
        this->read_XXRMC(nmea);
    });
    _parser.setSentenceHandler(getTag("VTG"), [this](const NMEASentence& nmea){
        this->read_XXVTG(nmea);
    });

}

void IService::read_PSRF150(const NMEASentence& nmea){
    // nothing right now...
    // Called with checksum 3E (valid) for GPS turning ON
    // Called with checksum 3F (invalid) for GPS turning OFF
}

void IService::read_XXGGA(const NMEASentence& nmea){
    /* -- EXAMPLE --
...
void IService::read_XXGSA(const NMEASentence& nmea){
    /*  -- EXAMPLE --
...
void IService::read_XXGSV(const NMEASentence& nmea){
    /*  -- EXAMPLE --
...
void IService::read_XXRMC(const NMEASentence& nmea){
    /*  -- EXAMPLE ---
...
void IService::read_XXVTG(const NMEASentence& nmea){
    /*
